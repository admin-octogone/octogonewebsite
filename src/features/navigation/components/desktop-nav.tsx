  ...
  return (
    <NavigationMenuLink asChild>
      <Link
        ...
        {...props}
      >
        <div className="text-lg font-semibold leading-tight text-marine-900">{title}</div>
        {children && <p className="text-sm leading-relaxed text-marine-500 mt-2">{children}</p>}
      </Link>
    </NavigationMenuLink>
  )
})
        ...
        {routes.map((route) => {
          ...
          return (
            <NavigationMenuItem key={route.path}>
              {hasChildren ? (
                <>
                  <NavigationMenuTrigger className={`${navigationTriggerVariants({ active: isActive })} nav-item ${isActive ? 'active-nav-item' : ''} rounded-md px-3 py-2`}>
                    {route.label}
